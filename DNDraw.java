 ...
 * Danny Ramírez
 *
 * Compilation: javac DNDraw.java
 * Execution: java DNDraw
 * Dependencies: none
 *
 * DNDraw (Standard drawing library).
...
public final class DNDraw implements ActionListener, MouseListener,
    ...
    private static DNDraw std = new DNDraw();
    ...
    private static Image getImage(String filename) {
        ...
        // in case file is inside a .jar (classpath relative to DNDraw)
        if ((icon == null) && (icon.getImageLoadStatus() != MediaTracker.COMPLETE))
        {
            URL url = DNDraw.class.getResource(filename);
            if (url != null)
                icon = new ImageIcon(url);
        }
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = DNDraw.userX(e.getX());
            mouseY = DNDraw.userY(e.getY());
            isMousePressed = true;
        }
    }
    ...
    public static void main(String[] args) {
        DNDraw.square(0.2, 0.8, 0.1);
        DNDraw.filledSquare(0.8, 0.8, 0.2);
        DNDraw.circle(0.8, 0.2, 0.2);

        DNDraw.setPenColor(DNDraw.GREEN);
        DNDraw.setPenRadius(0.02);
        DNDraw.arc(0.8, 0.2, 0.1, 200, 45);

        // draw a blue diamond
        DNDraw.setPenRadius();
        DNDraw.setPenColor(DNDraw.BLUE);
        double[] x = { 0.1, 0.2, 0.3, 0.2 };
        double[] y = { 0.2, 0.3, 0.2, 0.1};
        DNDraw.filledPolygon(x, y);

        // text
        DNDraw.setPenColor(DNDraw.BLACK);
        DNDraw.text(0.2, 0.5, "black text");
        DNDraw.setPenColor(DNDraw.GRAY);
        DNDraw.text(0.8, 0.8, "grey text");
    }
}

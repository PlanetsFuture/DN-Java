 ...
 *
 *         Compilation: javac DNStats.java
 *         Execution: java DNStats < input.txt
 ...
 *         input, and computes the mininum, mean, maxinum, and
 *         standard deviation.
 *
...
/*
 * The {@code DNStats} class provides statistic methods for computig
 * statistic such as min, max, mean, sample standard deviation, and
 * sample variance.
 */
public final class DNStats {
    ...
    public static double mean(double[] a, int lo, int hi) {
        ...
        double mean = sum(a, lo, hi) / length;
        return mean;
    }
    ...
    /**
     * Plots the points (0, a0), (0, a1), ...
     */
    public static void plotPoints(double[] a) {
        validateNotNull(a);

        int n = a.length;
        DNDraw.setXscale(-1, n);
        DNDraw.setPenRadius(1.0 / (3.0 * n));
        for (int i = 0; i < n; i++)
            DNDraw.point(i, a[i]);
    }
    ...
    public static void plotBars(double[] a) {
        ...
        DNDraw.setXscale(-1, n);
        for (int i = 0; i < n; i++) {
            DNDraw.filledRectangle(i, a[i] / 2, 0.25, a[i] / 2);
        }
    }

    /*
     * throw IllegalArgumentException if x is null
    ...
    // thrwo an exception unless 0 <= lo <= hi <= length
    ...
    /*
     * Unit test {@code DNStats}
     * 
     * Convert command-line arguments to array of doubles and call
     * various methods.
     *
    ...
    public static void main(String[] args) {
        ...
        DNOut.printf("        min %10.3f\n", min(a));
        DNOut.printf("       mean %10.3f\n", mean(a));
        DNOut.printf("        max %10.3f\n", max(a));
        DNOut.printf("     stddev %10.3f\n", stddev(a));
        DNOut.printf("        var %10.3f\n", var(a));
        DNOut.printf("    stddevp %10.3f\n", stddevp(a));
        DNOut.printf("       varp %10.3f\n", varp(a));
    }
}
